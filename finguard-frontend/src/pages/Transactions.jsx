...
export default function Transactions() {
  ...
  // ✅ Status priority logic (BLOCKED > FLAGGED > SUCCESS > PENDING)
  const getFinalStatus = (tx) => {
    const status = (tx?.status || "").toUpperCase();

    if (status.includes("BLOCK")) return "BLOCKED";
    if (status.includes("FLAG")) return "FLAGGED";
    if (status.includes("SUCCESS") || status.includes("APPROVED")) return "SUCCESS";
    if (status.includes("PENDING")) return "PENDING";

    return status || "SUCCESS";
  };

  const detectIpAddress = useCallback(
  ...
  async function fetchTransactions(silent = false) {
    try {
      ...
      // ✅ Deduplicate + keep highest priority status
      const map = new Map();
      (res.data || []).forEach((tx) => {
        ...
        if (!existing) {
          ...
          const oldStatus = getFinalStatus(existing);
          const newStatus = getFinalStatus(tx);
  ...
  useEffect(() => {
    let timeoutId;
    const hasPending = transactions.some((tx) => getFinalStatus(tx) === "PENDING");
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.success(
        `Transaction of ${newTransaction.amount} ${newTransaction.currency} submitted successfully!`,
        {
          theme: document.documentElement.classList.contains("dark-mode")
            ? "dark"
            : "light",
        ...
        {loading ? (
          ...
          <div className="table-box" style={{ overflowX: "auto" }}>
            <table className="data-table" style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {transactions.map((tx) => {
                  const finalStatus = getFinalStatus(tx);
                  ...
                  return (
                    <tr key={tx.id} style={{ borderBottom: "1px solid var(--border-color)" }}>
                      ...
                      <td style={{ padding: "12px" }}>
                        <span
                          className={`status-badge ${getStatusBadgeClass(finalStatus)}`}
                          style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: "6px",
                        ...
                        >
                          {finalStatus === "PENDING" && <span className="badge-spinner"></span>}
